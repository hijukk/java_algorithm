...
public class Java11FourSequenceMulti {

	public static int multiRight(int[][] arr) {
		int multi = 1;
		int max = 0;
	...
	public static int multiDown(int[][] arr) {
		int multi = 1;
		int max = 0;
	...
	public static int multiDiagonal(int[][] arr) {
		int multi = 1;
		int max = 0;
	...
	public static int multiLeftDiagonal(int[][] arr) {
		int multi = 1;
		int max = 0;
		for (int i = 0; i < arr.length - 3; i++) {
...
//		그러면 수평, 수직, 또는 대각선 방향으로 연속된 수 네 개의 곱 중 최댓값은 얼마입니까?
		...
		String[] ArraysString = StringNum.split(" ");
		int[][] arrs = new int[20][20];

		for (int i = 0; i < arrs.length; i++) {
			for (int x = 0; x < arrs.length; x++) {
				arrs[i][x] = Integer.parseInt(ArraysString[(i * 20) + x]);
			}
		}
...
//		}
		...
		int right = multiRight(arrs);
		System.out.println("right: " + right);

		int down = multiDown(arrs);
		System.out.println("down: " + down);

		int diagonal = multiDiagonal(arrs);
		System.out.println("diagonal:" + diagonal);
		
		int leftDiagonal = multiLeftDiagonal(arrs);
